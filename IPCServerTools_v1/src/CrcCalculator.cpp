...
// crc16校验
uint16_t server_global_functools_Crc::CalculateCrc16(const uint8_t* data, size_t length, uint16_t initialValue = 0xFFFF, uint16_t polynomial = 0xA001) {
...
bool server_global_functools_Crc::CutDataforcrc16(const std::vector<uint8_t>& recBytes) { //CutData功能：报文切割
    size_t DataFullLength = recBytes.size();
    size_t crcDatasIndex = 4;
    size_t crcDatasLength = DataFullLength - 8 - 2;
    
    // crc16算法调用
    server_global_functools_Crc crcCalculator;
